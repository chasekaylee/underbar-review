(function() {
  ...
  _.uniq = function(array, isSorted, iterator) {
    ...
    if (iterator) {
      _.each(array, function(el) {
        if (!(iteratedArr.includes(iterator(el)))) {
          iteratedArr.push(iterator(el));
          values.push(el);
        }
      });
    }
    
    array = iterator ? iteratedArr : _.identity(array);
    
    for (var i = 0; i < array.length; i++) {
      if (iterator) {
        unique[array[i]] = values[i];
      } else {
  ...
  _.some = function(collection, iterator) {
    ...
    return !!_.reduce(collection, function(allFalse, item) {
      if (iterator) {
        return allFalse || iterator(item); 
      }
      
      return allFalse || item;
    }, false);
  };
